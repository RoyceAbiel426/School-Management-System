...
export default function EditSport() {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        setFormData({
          ...
          coach: sport.coach?._id || sport.coach || "",
          captain: sport.captain?._id || sport.captain || "",
        });

        if (Array.isArray(sport.participants)) {
          const participantIds = sport.participants.map((p) => p._id || p);
          setSelectedParticipants(participantIds);
        }
      }
    } catch (err) {
      console.error("Error fetching sport:", err);
      setError(err.response?.data?.message || "Failed to load sport data");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await adminService.updateSport(id, sportData);
      ...
      setError(err.response?.data?.message || "Failed to update sport");
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <div>
        ...
        <p className="text-muted-foreground">Update sport information</p>
      </div>

      {/* Success Message */}
      {success && (
        <Alert type="success">Sport updated successfully! Redirecting...</Alert>
      )}
          ...
          <div className="space-y-4">
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                Update the list of students participating in this sport
              </p>
            </div>
